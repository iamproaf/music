...
from music import BOT_ID, BOT_USERNAME, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from music.Database import (add_gban_user, add_off, add_on, add_sudo,
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m music")
        else:
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            ...
            return os.system(f"kill -9 {os.getpid()} && python3 -m music")
